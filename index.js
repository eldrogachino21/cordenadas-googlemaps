...
marker.bindPopup(`<b>${element.nombre}</b><br><button onclick="editar(${element.nombre.toString()},${element.longitud},${element.latitud})">Editar</button>`).openPopup();
...
function onMapClick(e) {
    popup
        ...
        .setContent("Estas dando click en la cordenada " + e.latlng .toString()+ `<button onclick='Añadir(${e.latlng.toString().replaceAll("LatLng","").replaceAll("(","").replaceAll(")","").split(",")})'>Añadir</button>` )
        .openOn(map);
}
...
function editar (nombre,long,lat){
